...
**Purpose:** This log records every decision that amends or clarifies the *Technical Implementation Specification v1.0* (`docs/specification/technical-implementation-specification-v1.0.docx`).
...
| T-005 | **`passengerCount` accepts 9+** to trigger the multi-vehicle / human-routing branch (BC1); only the *single-vehicle quote* is blocked, not the input | Otherwise the explanatory 9+ branch could never render. | 🟡 Proposed |
